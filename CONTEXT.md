...
**Retrievability vs. relevance** — the load-bearing distinction. Hit@K measures **retrievability**: given a (query, expected_path) pair, did the retriever surface that path? It does **not** measure human-judged **relevance** (is the result actually a good answer?). When the golden set is mined from or hand-written against the indexed corpus (the default, self-indexed case), Hit@K is optimistic *by construction* — it rewards finding back what was indexed. Consequence: the harness is sound for **regression detection** (a delta between two runs on the same eval set is real) but **not** for certifying **absolute quality** (a high Hit@K does not prove retrieval is good). Certifying absolute quality requires a hand-labeled holdout of natural-language queries — the deferred reopen trigger in `DECISIONS.md` §2. Surfaced at runtime as the `CAVEAT` in `run.py` (printed + written to every result JSON).
